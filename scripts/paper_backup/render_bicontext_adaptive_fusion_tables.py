#!/usr/bin/env python3
"""Render Adaptive Fusion metrics and paired deltas against plain BiContext."""
...
ADAPTIVE_MODEL = "Final-Ours-BiContext-Adaptive-Fusion"
BASE_MODEL = "Final-Ours-BiContext-Mamba"
EXPERIMENT = "e1_bicontext_adaptive_fusion_5seed"
...
def main() -> int:
    ...
    new_mean = _read(new_dir / f"{EXPERIMENT}_metrics_mean_std.csv")
    new_seed = _read(new_dir / f"{EXPERIMENT}_metrics_per_seed.csv")
    standalone = new_dir / f"{EXPERIMENT}_macro_table.md"
    _write_table(
        standalone,
        new_mean,
        (("Ours BiContext + Adaptive Fusion", ADAPTIVE_MODEL),),
    )
    print(standalone)
    ...
    comparison = new_dir / "e1_bicontext_adaptive_fusion_vs_bicontext_macro_table.md"
    _write_table(
        comparison,
        reference_mean + new_mean,
        (
            ("Ours BiContext", BASE_MODEL),
            ("Ours BiContext + Adaptive Fusion", ADAPTIVE_MODEL),
        ),
    )
    ...
    adaptive_index = {
        ...
        for row in new_seed
        if row["model"] == ADAPTIVE_MODEL and row["dataset"] != "ALL_DATASETS_MACRO"
    }
    base_index = {
        (row["dataset"], int(row["seed"])): row
    ...
    if set(adaptive_index) != set(base_index):
        raise ValueError(
            "Adaptive Fusion and BiContext do not have identical dataset/seed coverage"
        )
    paired = []
    for key in sorted(adaptive_index):
        adaptive, base = adaptive_index[key], base_index[key]
        if (adaptive["n_cycles"], adaptive["n_batteries"]) != (
            base["n_cycles"], base["n_batteries"]
        ):
        ...
        paired.append(
            {
                "dataset": key[0],
                "seed": key[1],
                "adaptive_minus_bicontext_macro_mape_percent": (
                    float(adaptive["battery_macro_mape_percent"])
                    - float(base["battery_macro_mape_percent"])
                ),
                "adaptive_minus_bicontext_macro_rmse_percent": (
                    float(adaptive["battery_macro_rmse_soh_percent"])
                    - float(base["battery_macro_rmse_soh_percent"])
                ),
            }
        )
    paired_columns = (
    ...
    paired_path = new_dir / "e1_adaptive_fusion_vs_bicontext_per_seed.csv"
    ...
    for dataset in sorted({row["dataset"] for row in paired}):
        ...
        mape = [row["adaptive_minus_bicontext_macro_mape_percent"] for row in selected]
        rmse = [row["adaptive_minus_bicontext_macro_rmse_percent"] for row in selected]
        aggregate.append(
            {
                "dataset": dataset,
    ...
    aggregate_columns = (
        "dataset",
        "seed_count",
        "macro_mape_delta_mean",
        "macro_mape_delta_std",
        "macro_mape_win_seeds",
        "macro_rmse_delta_mean",
        "macro_rmse_delta_std",
        "macro_rmse_win_seeds",
    )
    aggregate_path = new_dir / "e1_adaptive_fusion_vs_bicontext_mean_std.csv"

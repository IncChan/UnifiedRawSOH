...
if [[ "${EXPERIMENT_SUITE}" == "e1_final_interaction_5seed" ]]; then
  ...
  AVAILABLE_MODELS=(hi_mlp smaller_transformer ours_pointbridge)
  DEFAULT_OUTPUT_ROOT="${REPO_ROOT}/outputs/Paper-Backup/CRateV2-128x128"
  DEFAULT_PATIENCE=20
  DEFAULT_BATCH_SIZE=128
  ...
  AVAILABLE_MODELS=(ours_dominant ours_fullvi ours_gated ours_pointbridge smaller_transformer transformer)
  DEFAULT_OUTPUT_ROOT="${REPO_ROOT}/outputs/Paper-Backup/CRateV2-128x128"
  DEFAULT_PATIENCE=20
  DEFAULT_BATCH_SIZE=128
  DEFAULT_NUM_WORKERS=1
  DEFAULT_SEEDS="42 52 62"
  DEFAULT_EPOCHS=400
elif [[ "${EXPERIMENT_SUITE}" == "e1_shared_crate_fullvi" ]]; then
  AVAILABLE_MODELS=(ours_dominant ours_fullvi smaller_transformer transformer)
  DEFAULT_OUTPUT_ROOT="${REPO_ROOT}/outputs/Paper-Backup/CRateV2"
  DEFAULT_PATIENCE=20
  DEFAULT_BATCH_SIZE=128
  DEFAULT_NUM_WORKERS=1
  DEFAULT_SEEDS="42 52 62"
  ...
  AVAILABLE_MODELS=(hi_mlp ours transformer)
...
if (( ${#REQUESTED_MODELS[@]} == 1 )) && [[ "${REQUESTED_MODELS[0],,}" == "all" ]]; then
  ...
  for requested in "${REQUESTED_MODELS[@]}"; do
    requested="${requested,,}"
    [[ "${requested}" != "all" ]] || {
      echo "MODELS=all cannot be combined with explicit E1 models." >&2
      exit 2
    }
    found=0
    ...
    (( found == 1 )) || {
      echo "Unknown E1 model '${requested}'. Allowed: all ${AVAILABLE_MODELS[*]}" >&2
      exit 2
    }
    duplicate=0
...
(( ${#SELECTED_MODELS[@]} > 0 )) || {
  echo "MODELS must select at least one E1 model." >&2
  exit 2
}
mapfile -t CONFIGS < <(
...
echo "Paper-Backup E1 dynamic launcher"
...
# prevents a missing manifest/split from turning into many identical failures.
config_index=0
for config in "${CONFIGS[@]}"; do
  ...
  [[ -n "${EPOCHS:-}" ]] && validate_args+=(--epochs "${EPOCHS}")
  [[ -n "${PATIENCE:-}" ]] && validate_args+=(--patience "${PATIENCE}")
  [[ -n "${BATCH_SIZE:-}" ]] && validate_args+=(--batch_size "${BATCH_SIZE}")
...
while (( next_task < task_count || active_jobs > 0 )); do
  ...
  # with one physical GPU, so per-GPU concurrency can never exceed the limit.
...
if (( overall_status != 0 )); then
  echo "one or more Paper-Backup E1 jobs failed; inspect ${LAUNCHER_ROOT}" >&2
  exit 1
fi
...
if [[ "${EXPERIMENT_SUITE}" == "e1_core3_128x128" ]]; then
  ...
  echo "summary: ${PYTHON_BIN} ${SCRIPT_DIR}/summarize_results.py --experiment e1_crate --seeds '${SEED_LIST[*]}' --root '${OUTPUT_ROOT}' --output-dir '${OUTPUT_ROOT}/summaries'"
else
  echo "summary: ${PYTHON_BIN} ${SCRIPT_DIR}/summarize_results.py --experiment e1 --seeds '${SEED_LIST[*]}'"
fi

...
STAGE="${1:-all}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/outputs/Paper-Backup/E1-Late-LatentToken-BiContext-ReZero-5Seed}"
...
LATE_TOKEN_ROOT="${LATE_TOKEN_ROOT:-${REPO_ROOT}/outputs/Paper-Backup/E1-Late-LatentToken-BiContext-5Seed}"
SEED_SPEC="${SEEDS:-42 52 62 72 82}"
RUN_TAG="${RUN_TIME:-e1_late_latent_token_bicontext_rezero_5seed_$(date +%Y%m%dT%H%M%S)}"
...
validate_all() {
  ...
  OUTPUT_ROOT="${OUTPUT_ROOT}" PYTHON_BIN="${PYTHON_BIN}" \
    ...
    bash "${SCRIPT_DIR}/run_e1_late_latent_token_bicontext_rezero_5seed.sh"
}
...
summarize_all() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/render_late_latent_token_bicontext_rezero_tables.py" \
    ...
    --late-token-summary-dir "${LATE_TOKEN_ROOT}/summaries"
}

...
# Full Vanilla jobs remain blocked until a real point-level full source is configured.
...
RUN_TIME="${RUN_TIME:-e2_charging_information}"
CHECK_DATA="${CHECK_DATA:-1}"

read -r -a SEED_LIST <<< "${SEEDS//,/ }"
mapfile -t CONFIGS < <(find "${REPO_ROOT}/configs/paper_backup/e2_charging_information" -type f -name '*.json' | sort)

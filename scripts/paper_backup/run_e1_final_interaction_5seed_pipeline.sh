...
STAGE="${1:-all}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/outputs/Paper-Backup/E1-Final-Interaction-5Seed}"
SEED_SPEC="${SEEDS:-42 52 62 72 82}"
RUN_TAG="${RUN_TIME:-e1_final_interaction_5seed_$(date +%Y%m%dT%H%M%S)}"
...
validate_all() {
  ...
  OUTPUT_ROOT="${OUTPUT_ROOT}" PYTHON_BIN="${PYTHON_BIN}" \
    ...
    bash "${SCRIPT_DIR}/run_e1_final_interaction_5seed.sh"
}
...
summarize_all() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/summarize_results.py" \
    --experiment e1_final_interaction_5seed --seeds "${SEED_SPEC}" \
    --root "${OUTPUT_ROOT}" --output-dir "${summary_root}"
  "${PYTHON_BIN}" "${SCRIPT_DIR}/render_final_interaction_tables.py" \
    --experiment e1 \
    --input "${summary_root}/e1_final_interaction_5seed_metrics_mean_std.csv" \
    --output "${summary_root}/e1_final_interaction_5seed_macro_table.md"
}

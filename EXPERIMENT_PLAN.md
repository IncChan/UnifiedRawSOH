...
|---|---|---|---|
| E1 Raw SOH Learning | Can raw CC/CV/T learn SOH within a domain? Is the proposed phase-specific design needed? | configs/e1_raw_soh_learning | XJTU and SmartHealth RawMamba/Only-F runnable; MIT is enabled once its phase-aware local physical124 export is nonempty |
| E2 Unified Multi-domain Learning | Can one model serve multiple battery domains? | configs/e2_unified_multidomain | loader/config supports all public domains; no formal Unified run is started in this task |
| E3 Cross-domain Reusability | Does a pretraining-unseen domain benefit zero-shot or with the same few-shot/few-cell budget? | configs/e3_cross_domain_reusability | protocol/config validation ready; transfer trainer not yet implemented |
| E4 Industrial External Validation | Can public unified pretraining transfer to enterprise families? | configs/e4_industrial_external | interface only; requires real enterprise data |

Contrastive learning is not a fifth group. It is a conditional extension under
configs/conditional/health_aware_contrastive, triggered only by an E2
diagnosis.
...
RMSE. It reports per-domain metrics as well as aggregate diagnostics.
...
The following only validate wiring. They are not formal paper training:
    ...
    python -m UnifiedRawSOH.main --config UnifiedRawSOH/configs/e1_raw_soh_learning/benchmark/raw_mamba_xjtu.json --backend_override torch_reference --device_override cpu --epochs 1 --patience 1 --debug_num_samples 1 --run_time smoke
    python -m UnifiedRawSOH.main --config UnifiedRawSOH/configs/e1_raw_soh_learning/benchmark/raw_mamba_mit.json --backend_override torch_reference --device_override cpu --epochs 1 --patience 1 --debug_num_samples 1 --run_time smoke

For intentional multi-seed CUDA training, edit fixed settings at the top of
the corresponding shell script:

    scripts/e1_raw_soh_learning/benchmark/run_raw_mamba_benchmark.sh
    scripts/e1_raw_soh_learning/benchmark/run_onlyf_benchmark.sh
    scripts/e1_raw_soh_learning/ablation/run_raw_mamba_ablation.sh
    scripts/e2_unified_multidomain/run_public_xjtu_mit.sh

No script in this restructuring task launches a formal training run.

...
def _forward_prediction(model_type: str, model: torch.nn.Module, batch: Mapping[str, Any], device: torch.device) -> torch.Tensor:
    ...
    if kind == "sequence":
        return model(batch["sequence"].to(device), batch.get("mask", None).to(device) if isinstance(batch.get("mask"), torch.Tensor) else None)
    inputs = {key: batch[key].to(device) for key in PHASE_KEYS}
...
def run_epoch(
    ...
    *,
    ...
    with context:
        for batch in loader:
            truth = batch["soh"].to(device).reshape(-1, 1)
    ...
    metrics["loss"] = loss_sum / sample_count if sample_count else float("nan")
...
def train_from_config(
    ...
    *,
    ...
    for epoch in range(1, max_epochs + 1):
        train_metrics, _ = run_epoch(model_type, model, loaders["train"], actual_device, optimizer=optimizer, collect_predictions=True)
        with torch.no_grad():
        ...
        if improved:
            ...
            no_improvement += 1
        history.append({"epoch": epoch, "train": train_metrics, "val": val_metrics, "monitor": monitored, "best": improved})
    ...
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "best_epoch": best_epoch}, run_dir / "best.pt")

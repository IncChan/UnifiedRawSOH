...
from UnifiedRawSOH.preprocess.smvic_common import FAMILY_SPECS  # noqa: E402


DEFAULT_ROOT = REPO_ROOT / "datasets" / "SMVIC_preprocessed_v2_128x128"
...
def _model_smoke(cc: np.ndarray, cv: np.ndarray, features: np.ndarray) -> dict:
    ...
    if mlp_out.shape != (count, 1) or bicontext_out.shape != (count, 1):
        raise ValueError("Unexpected target-model output shape")
    if not torch.isfinite(mlp_out).all() or not torch.isfinite(bicontext_out).all():
...
def validate_domain(directory: Path, skip_model_smoke: bool) -> dict:
...
def main() -> int:
    ...
    results = {
        domain: validate_domain(args.input_root / domain, args.skip_model_smoke)

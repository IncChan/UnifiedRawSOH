...
class StandardMambaPhaseInputEncoder(nn.Module):
    ...
    def __init__(self, input_dim, d_model, use_time_as_input, temperature_injection, temperature_features, time_scale_min):
        super().__init__()
        ...
        expected_input_dim = 2 + 1 + self.temperature_channels
        if self.input_dim != expected_input_dim:
            raise ValueError(
                f"Phase token expects input_dim={expected_input_dim} for {key}; got {self.input_dim}."
            )
...
class PaperRawSOHModel(nn.Module):
    ...
    def __init__(
        ...
        expected_input_dim = 2 + 1 + temperature_token_channels
        if int(input_dim) != expected_input_dim:
            raise ValueError(
                f"input_dim must be {expected_input_dim}: signal(2)+time(1)+configured temperature channels "
        ...
        if use_cc_to_cv_bridge and (
            ...
            raise ValueError("C5B bridge must be zero-init Linear(64, 32).")
        ...
        self.cc_branch = StandardMambaPhaseBranch(**branch_kwargs)
        self.cv_branch = StandardMambaPhaseBranch(**branch_kwargs)

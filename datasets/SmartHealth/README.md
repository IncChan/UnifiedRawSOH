# SmartHealth canonical CC/CV/calibration-SOH v7
...
temperature data. The v7 implementation is split into six family-specific
commands under `UnifiedRawSOH/preprocess/`; the resulting canonical RAW and
...
## Auditable policy (`smarthealth_cccv_calibration_v7`)
...
2. The principal charge event is the longest contiguous `恒流恒压充电` event.
   ...
   persists for 5 source points, is at least 1% below the early 90th-percentile
   CC reference, and is within 0.02 V of that cycle's charge-voltage maximum.
   At least 8 inferred CC and 8 inferred CV points are required; there is no
   EVE/DOD-specific 30-point gate.
   Model points are then selected only from inferred CC `3.45–3.58 V` and
   inferred CV `0.25C–0.05C`, using nominal capacity for C-rate. When adjacent
   source points bracket a fixed endpoint, one traceable linearly interpolated
   endpoint is exported with both bracket row indices; the physical window is
   not widened to a nearby sample.
3. Discharge capacity is the `max - min` span of `放电容量(Ah)` in the principal
   ...
   `Q_ref=median(first three)`. Partial-DOD full-capacity cycles are anchor-only
   and never enter RAW/FEATURE; normal partial-DOD cycles use bounded linear
   interpolation. 100%DOD cycles remain direct-labelled model inputs. RUL,
   EOL, and extrapolation are not generated.
...
The superseded v6 detector is frozen under
`preprocess/legacy/smarthealth_v6_boundary_first/`; canonical v7 does not
import it.
